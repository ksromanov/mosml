...
(*
   [word] is the type of 32-bit unsigned words, or 32-bit unsigned
   integers in the range 0..4294967295.  Represented as a subset of
   the native word type (63 bits on 64-bit systems).
   ...
   multiplication, division, and remainder, modulus 2^32.  The
   ...
   [toInt w] returns the integer in the range 0..4294967295 represented
   by w.  Never raises Overflow on 64-bit systems.

   [toIntX w] returns the signed integer (in the range
   ~2147483648..2147483647) represented by two's complement
   bit-pattern w.

   [fromInt i] returns the word holding the 32 least significant bits of i.

   [toLargeInt w] returns the integer represented by w.

   [toLargeIntX w] returns the signed integer represented by two's
   complement bit-pattern w.

   [fromLargeInt i] returns the word holding the 32 least significant
   bits of i.
   ...
   [fromLarge w] returns w modulo 2^32.
