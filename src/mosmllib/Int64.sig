...
(*
   [int] is the type of 64-bit signed integers, represented internally
   as a pair (hi, lo) of native ints, where hi holds the upper 32 bits
   and lo holds the lower 32 bits (unsigned, 0 to 4294967295).

   [precision] is SOME 64.

   [minInt] is SOME (~2147483648, 0), representing ~9223372036854775808.

   [maxInt] is SOME (2147483647, 4294967295), representing 9223372036854775807.

   [~]
   [*]
   [+]
   [-] are the usual operations on integers.  They raise Overflow if
   the result is not representable as a 64-bit integer.

   [abs] returns the absolute value of its argument.  Raises Overflow
   if applied to minInt.

   [div] is integer division, rounding towards minus infinity.
   Evaluating i div 0 raises Div.  Evaluating i div ~1 raises
   Overflow if i is minInt.
   ...
   [toInt x] converts x to the default int type.  Raises Overflow
   if the value does not fit in 63 bits.

   [fromInt x] converts x from the default int type.  This always
   succeeds since 63-bit values fit within 64-bit range.
